...
public class DrivingRouteOverlay extends OverlayManager {
    ...
    public final List<OverlayOptions> getOverlayOptions() {
        ...
        if (mRouteLine.getStarting() != null) {
            ...
            bundle.putInt(MapConstant.MARKER_CLICK, MapConstant.START_MARKER_CLICK);
            overlayOptionses.add((new MarkerOptions())
                    .position(mRouteLine.getStarting().getLocation())
                    .icon(getStartMarker() != null ? getStartMarker() :
                            BitmapDescriptorFactory
                                    .fromAssetWithDpi("Icon_start.png")).zIndex(10).extraInfo(bundle));
        }
        if (mRouteLine.getTerminal() != null) {
            Bundle bundle = new Bundle();
            bundle.putInt(MapConstant.MARKER_CLICK, MapConstant.TERMINAL_MARKER_CLICK);
            overlayOptionses
...
//            }
    ...
    /**
     * 覆写此方法以改变 起点 默认点击行为 -zlp
     */
    public void onStartMarkerClick() {
    }

    /**
     * 覆写此方法以改变 终点 默认点击行为 -zlp
     */
    public void onTerminalMarkerClick() {
    }

    @Override
    public final boolean onMarkerClick(Marker marker) {
        for (Overlay m : getOverlays()) {
            if (m instanceof Marker && m.equals(m)) {
                if (m.getExtraInfo() != null) {
                    if (m.getExtraInfo().getInt(MapConstant.MARKER_CLICK, -1) == MapConstant.START_MARKER_CLICK) {
                        onStartMarkerClick();
                        continue;
                    } else if (m.getExtraInfo().getInt(MapConstant.MARKER_CLICK, -1) == MapConstant.TERMINAL_MARKER_CLICK) {
                        onTerminalMarkerClick();
                        continue;
                    }
                    onRouteNodeClick(m.getExtraInfo().getInt("index"));
                }
            }
        }
        return true;
    ...
    public boolean onPolylineClick(Polyline polyline) {
        boolean flag = false;
        for (Overlay mPolyline : getOverlays()) {
