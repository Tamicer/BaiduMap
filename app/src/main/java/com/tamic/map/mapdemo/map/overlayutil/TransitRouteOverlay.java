...
//                    color = Color.argb(178, 88, 208, 0);
    ...
    public final boolean onMarkerClick(Marker marker) {
        for (Overlay mMarker : getOverlays()) {
            if (mMarker instanceof Marker && mMarker.equals(marker)) {
                if (marker.getExtraInfo() != null) {
                    // MARKER_CLICK -zlp
                    if (marker.getExtraInfo().getInt(MapConstant.MARKER_CLICK, -1) == MapConstant.START_MARKER_CLICK) {
                        onStartMarkerClick();
                        continue;
                    } else if (marker.getExtraInfo().getInt(MapConstant.MARKER_CLICK, -1) == MapConstant.TERMINAL_MARKER_CLICK) {
